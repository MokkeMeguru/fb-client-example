...
* [`src/clj/client_example/`](src/clj/client_example/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/client_example/`](src/cljs/client_example/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/client_example/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/client_example/`](test/cljs/client_example/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `client-example` project root directory
...
in the [`src/clj/client_example/`](src/clj/client_example/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/client_example/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns client-example.example
  (:require [client-example.config :as config])
